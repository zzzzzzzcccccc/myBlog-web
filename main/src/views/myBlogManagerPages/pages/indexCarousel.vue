        ...
        <el-table-column v-for="(item, index) in table"
          ...
          <template slot-scope="scope">
            <el-tooltip class="font-over" effect="dark" :content="'' + scope.row[item.name]" placement="top-start">
              <span class="cell-inner" v-if="item.name === 'imgUrl'">
    ...
    <section class="item-dialog">
      <el-dialog :visible="dialogVisible" title="填写历程类型信息" width="30%" @close="bindDialogClose">
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" status-icon label-width="80px" label-position="right">
          ...
          <el-form-item label="代码地址" prop="imgUrl">
...
<script>
  import { globalBannerService } from '../../../utils/service'
  ...
  export default {
    data () {
      return {
        ...
        rules: {
          ...
          imgUrl: [{ required: true, message: '请输入图片地址' }]
        },
        loading: false
      }
    },
    ...
    methods: {
      ...
      doGetData () {
        globalBannerService.list({ cb: data => this.tableData = data })
      },
      bindAdd () {
      ...
      bindDelete (index, row) {
        showConfirm({
          cb: () => {
            globalBannerService.deleteOne(row.id).then(({ code, msg }) => {
      ...
      async doAddOne () {
        this.loading = true
        const { code, msg, data } = await globalBannerService.addOne(this.ruleForm)
        this.loading = false
        if (code === enums.SUCCESS_CODE) {
      ...
      async doUpdateOne () {
        this.loading = true
        const { code, msg, data } = await globalBannerService.updateOne(this.ruleForm)
        this.loading = false
        if (code === enums.SUCCESS_CODE) {
