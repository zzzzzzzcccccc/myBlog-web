  ...
  <div>
        ...
        <el-table-column v-for="(item, index) in table"
          ...
          <template slot-scope="scope">
            <el-tooltip class="font-over" effect="dark" :content="'' + scope.row[item.name]" placement="top-start">
              <span class="cell-inner" v-if="item.name === 'href'">
    ...
    <section class="item-dialog">
      <el-dialog :visible="dialogVisible" title="填写历程类型信息" width="30%" @close="bindDialogClose">
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" status-icon label-width="100px" label-position="right">
          ...
          <el-form-item label="图标样式名" prop="iconClassName">
            <el-input v-model="ruleForm.iconClassName"></el-input>
          </el-form-item>
...
<script>
  import { globalNavService } from '../../../utils/service'
  ...
  export default {
    data () {
      return {
        table: [
          ...
          {name: 'href', label: '链接'},
          {name: 'iconClassName', label: '图标样式名'},
    ...
    methods: {
      ...
      doGetData () {
        globalNavService.list({ cb: data => this.tableData = data })
      },
      bindAdd () {
      ...
      bindDelete (index, row) {
        showConfirm({
          cb: () => {
            globalNavService.deleteOne(row.id).then(({ code, msg }) => {
      ...
      async doAddOne () {
        this.loading = true
        const { code, msg, data } = await globalNavService.addOne(this.ruleForm)
        this.loading = false
        if (code === enums.SUCCESS_CODE) {
      ...
      async doUpdateOne () {
        this.loading = true
        const { code, msg, data } = await globalNavService.updateOne(this.ruleForm)
        this.loading = false
        if (code === enums.SUCCESS_CODE) {
